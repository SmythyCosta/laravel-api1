...
class ReportController extends Controller
{
    ...
    public function purchaseReportDownloadExcel(Request $request)
    {          
        $setting = Setting::where('id',1)->first();
    
        Excel::create('purchase-report', function ($excel) use ($request)   {
           
            $excel->setTitle('Purchase Report');
            $excel->sheet('Purchase Report', function ($sheet) use ($request) {
                ...
                // Set all borders (top, right, bottom, left)
                // $cells->setBorder('solid', 'none', 'none', 'solid');
                $sheet->row(1, array('Purchase Report'));
